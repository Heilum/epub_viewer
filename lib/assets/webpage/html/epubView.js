...
var isKeyboardVisible = false; // Track keyboard state to prevent re-renders
var keyboardDebounceTimer = null;
...
function loadBook(data, cfi, manager, flow, spread, snap, allowScriptedContent, direction, useCustomSwipe, backgroundColor, foregroundColor, fontSize, clearSelectionOnNav) {
  ...
  // Detect keyboard visibility changes to prevent re-renders
  var lastViewportHeight = window.innerHeight;
  window.addEventListener('resize', function() {
    var currentHeight = window.innerHeight;
    var heightDiff = Math.abs(currentHeight - lastViewportHeight);
    
    // If height changed significantly (>100px), likely keyboard show/hide
    if (heightDiff > 100) {
      console.log('[EPUB] Keyboard event detected, blocking re-renders');
      ...
      // Unblock after keyboard animation completes
      keyboardDebounceTimer = setTimeout(function() {
        isKeyboardVisible = false;
        console.log('[EPUB] Keyboard event settled, re-renders unblocked');
      }, 300);
    }
  ...
  rendition.on("displayed", function (renderer) {
    ...
    if (isAddingUnderline || isKeyboardVisible) {
      return;
    }
  });
  ...
  rendition.hooks.content.register((contents) => {
    ...
    function fixUnderlineStyles() {
      // Skip if we're in the middle of adding an underline or keyboard is visible
      if (isAddingUnderline || isKeyboardVisible) {
        return;
      }
