...
function loadBook(data, cfi, manager, flow, spread, snap, allowScriptedContent, direction, useCustomSwipe, backgroundColor, foregroundColor, fontSize, clearSelectionOnNav, axis, fontFamily, margin, pendingInitialSettings) {
    ...
    if (isIOS) {
      ...
      if (currentFlow == "paginated") {
        ...
        contents.document.addEventListener('touchend', function (e) {
          ...
          const isTap = deltaX < 5 && deltaY < 5 && duration < 100;

          console.log('[EPUB] Touch metrics - deltaX:', deltaX, 'deltaY:', deltaY, 'duration:', duration, 'isTap:', isTap);
      ...
      if (currentFlow == "paginated") {
        ...
        contents.document.addEventListener('touchstart', function (e) {
          ...
          touchStartTime = Date.now();
          isDragging = false;
          console.log('[EPUB] [Android] touchstart at', touchStartX, touchStartY);
        }, true);
        ...
        contents.document.addEventListener('touchend', function (e) {
          ...
          console.log('[EPUB] [Android] Touch metrics - deltaX:', deltaX, 'deltaY:', deltaY, 'duration:', duration, 'isTap:', isTap);
  ...
  book.ready.then(async function () {
    ...
    try {
      ...
      await book.locations.generate(256); // 512 characters per location (more precise)
