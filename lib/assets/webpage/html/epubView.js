  ...
  rendition.on("displayed", function (renderer) {
    ...
    rendition.hooks.content.register(function (contents) {
        ...
        doc.addEventListener('click', function(e) {
            var target = e.target;
            // Traverse up to find <a> tag if clicked on a child element
            while (target && target.tagName !== 'A') {
                target = target.parentElement;
            }
            
            if (target && target.tagName === 'A' && target.href) {
                // Send the href to Flutter
                // We use 'linkClicked' as the handler name
                console.log('[EPUB] Link clicked:', target.href);
...
async function loadFontAsDataURI(fontFamily) {
    ...
    try {
        // console.log('[EPUB] Fetching font:', fileName);
        ...
        console.error('[EPUB] Failed to load font ' + fileName, e);
        return null;
    }
}
