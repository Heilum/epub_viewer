...
function addHighlight(cfiRange, color, opacity) {
  ...
  // Modify the SVG elements to create wavy underline effect
  setTimeout(function () {
    if (annotation && annotation.mark && annotation.mark.element) {
      var element = annotation.mark.element;
      console.log('[EPUB] Applying wavy underline styles to highlight element:', element);

      // Make all rect elements transparent (no background)
      ...
      // Add wavy path elements at the bottom of each rect to create wavy underline effect
      rects.forEach(function (rect) {
        ...
        var waveHeight = 3; // Height of the wave
        var waveLength = 8; // Length of each wave cycle
        var pathData = 'M ' + x1 + ' ' + y;
      ...
      console.error('[EPUB] Failed to find highlight element for styling');
    }
  }, 50);
}
