...
function loadBook(data, cfi, manager, flow, spread, snap, allowScriptedContent, direction, useCustomSwipe, backgroundColor, foregroundColor, fontSize, clearSelectionOnNav) {
  ...
  rendition.hooks.content.register(function (contents) {
    ...
    contents.window.document.addEventListener('selectionchange', function () {
      ...
      if (!text) {
        ...
        // 这里可以通知 Flutter：selectionCleared
      ...
      if (!isSelecting) {
        isSelecting = true;
        // 这里可以通知 Flutter：selectionStart
        window.flutter_inappwebview.callHandler('selectionStart');
      }

      // 每次 selectionchange 都重启一个“静止计时器”
      clearTimeout(selectionEndTimer);
      selectionEndTimer = setTimeout(function () {
        // 一段时间没有新的 selectionchange，认为“用户已经选完 & 松手/停住”
        const finalSel = contents.window.getSelection();
        const finalText = finalSel ? finalSel.toString() : '';
        if (!finalText) return;

        // 这里当成 selectionEnd：一次性回调 Flutter
        sendSelectionData(getCfiFromSelection(contents), contents, 'selectionEnd');
        isSelecting = false;
        selectionEndTimer = null;
      }, 1000); // 200ms 可按体验调整
    });
    ...
    //   console.log('JS 手指抬起 [EPUB] cfiRange: ', cfiRange);
    //   if (!cfiRange) {
    //     //不能由js主动清
    //     //clearSelection();
    //     return;
    //   }
    //   sendSelectionData(cfiRange, contents, 'selectionEnd');
    // }

    // contents.document.addEventListener('pointerup', handleSelectionEnd, true);
    // contents.document.addEventListener('pointercancel', handleSelectionEnd, true);
  ...
  rendition.on("relocated", function (location) {
    ...
    if (clearSelectionOnPageChange && flow === 'scrolling') {
      if (selectionEndTimer) {
        clearTimeout(selectionEndTimer);
        selectionEndTimer = null;
      }
