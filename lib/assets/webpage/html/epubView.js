...
function loadBook(data, cfi, manager, flow, spread, snap, allowScriptedContent, direction, useCustomSwipe, backgroundColor, foregroundColor, fontSize, clearSelectionOnNav, axis, fontFamily, margin) {
  ...
  book.ready.then(function () {
    ...
    currentFontFamily = fontFamily || 'PingFangSC-Regular';
    // currentLineSpacing is already 1.2 by default, will be updated by setLineSpacing if called.

    // Use dynamic location generation
    updateLocations().then(() => {
      if (cfi) {
        rendition.display(cfi)
      }
      // safe to let updateLocations trigger the handler.
    })
  })
...
function getPageNumberFromCfi(cfi) {
  console.log('[EPUB] getPageNumberFromCfi called for:', cfi);

  if (!book) {
    console.error('[EPUB] Book not initialized');
    return -1;
  }

  if (!book.locations) {
    console.error('[EPUB] book.locations is undefined');
    return -1;
  }

  // Check if locations are actually generated
  if (book.locations.length() === 0) {
    console.warn('[EPUB] Locations not generated yet (length=0).');
    return -1;
  }

  try {
    // locationFromCfi returns an index (0-based)
    var index = book.locations.locationFromCfi(cfi);
    console.log('[EPUB] cfi:', cfi, '-> index:', index);

    if (typeof index !== 'number' || index < 0) {
      console.warn('[EPUB] locationFromCfi returned invalid index:', index);
      return -1;
    }
    return index + 1;
  } catch (e) {
    console.error('[EPUB] Error getting page number:', e);
    return -1;
  }
}
...
// Helper to regenerate locations based on font size, line spacing, and font family
// Formula: chars = 1600 * (20 / fontSize)^2 * (1.2 / lineSpacing) * (1.0 / fontFactor)
var locationDebounce;
function updateLocations() {
  var fSize = currentFontSize;
  var lSpacing = currentLineSpacing;
  var fFamily = currentFontFamily;

  var baseChars = 1600;
  var baseSize = 20;
  var baseSpacing = 1.2;
  var fontFactor = fontWidthFactors[fFamily] || 1.0;

  // Calculate factor
  var sizeFactor = Math.pow(baseSize / fSize, 2);
  var spacingFactor = baseSpacing / lSpacing;
  var widthFactor = 1.0 / fontFactor;

  // [NEW] Image Compensation
  // Images take up visual space but 0 "text characters".
  // To compensate, we reduce the "chars per page" target, effectively creating more pages
  // to account for the image content.
  var imageFactor = 1.0;
  try {
    if (book && book.package && book.package.manifest) {
      var manifest = book.package.manifest;
      var imageCount = 0;
      for (var key in manifest) {
        if (manifest[key]['media-type'] && manifest[key]['media-type'].startsWith('image/')) {
          imageCount++;
        }
      }
      console.log('[EPUB] Image count in manifest:', imageCount);

      // Heuristic: Reduce char limit by 0.1% per image, up to 40% max reduction.
      // e.g. 100 images => 10% reduction.
      // This is a rough approximation.
      var reduction = Math.min(imageCount * 0.001, 0.4);
      imageFactor = 1.0 - reduction;
    }
  } catch (e) {
    console.error('[EPUB] Error counting images:', e);
  }

  var newChars = Math.floor(baseChars * sizeFactor * spacingFactor * widthFactor * imageFactor);

  // Clamp to reasonable limits
  if (newChars < 200) newChars = 200;
  if (newChars > 6000) newChars = 6000;

  console.log('[EPUB] Generating locations. FontSize:', fSize, 'LineSpacing:', lSpacing, 'Font:', fFamily, 'Factor:', fontFactor, 'ImageFactor:', imageFactor, 'Chars:', newChars);
  return book.locations.generate(newChars).then(function () {
    console.log('[EPUB] Locations generated. Total pages:', book.locations.length());
...
function setFontSize(fontSize) {
  ...
  // Debounce location generation (expensive)
...
// Heuristic factors for font width relative to PingFang
// > 1.0 means wider font (fewer chars per page)
// < 1.0 means narrower font (more chars per page)
var fontWidthFactors = {
  'PingFangSC-Regular': 1.0,
  'FZLanTYJW_Zhun': 1.0,   // Round typically similar to Sans
  'FZZHUNYSJW': 0.95,      // Song/Serif often slightly more compact
  'FZKTJW': 1.1            // KaiTi often wider/looser
};
