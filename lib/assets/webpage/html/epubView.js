...
// selectedCfi: CFI from the viewer's text selection (always resolvable)
// entryCfis: array of CFI strings from JSON entries (may not resolve if DOM differs)
// entryTexts: array of text strings from JSON entries (for fallback positioning)
function findMatchingAudioEntry(selectedCfi, entryCfis, entryTexts) {
  try {
    // Get the current chapter's document
    var contents = rendition.getContents();
    if (!contents || contents.length === 0) return -1;
    var doc = contents[0].document || contents[0].content;
    var body = doc.body || doc.querySelector('body');
    if (!body) return -1;
    var fullText = body.textContent || '';

    // Normalize CFI: strip "/2" (html root) after "!"
    function normalizeCfi(cfi) {
      return cfi.replace(/!\/2\//g, '!/');
    }

    // Try to resolve a CFI to a DOM Range
    function resolveRange(cfi) {
      try { var r = rendition.getRange(cfi); if (r) return r; } catch(e) {}
      var n = normalizeCfi(cfi);
      if (n !== cfi) { try { var r = rendition.getRange(n); if (r) return r; } catch(e) {} }
      var w = cfi.replace(/!\//, '!/2/');
      if (w !== cfi && w !== n) { try { var r = rendition.getRange(w); if (r) return r; } catch(e) {} }
      return null;
    }

    // Compute the global character offset of a Range's start in the body
    ...
    // 1. Resolve selected CFI and compute its global char offset
    var selectedRange = resolveRange(selectedCfi);
    if (!selectedRange) {
      console.error('[EPUB] findMatchingAudioEntry: cannot resolve selected CFI');
      return -1;
    }
    var selectedOffset = globalCharOffset(selectedRange);
    console.log('[EPUB] selectedOffset=' + selectedOffset);
    if (selectedOffset < 0) return -1;

    // 2. For each entry, get its global offset (CFI first, text fallback)
    var bestIdx = -1;
    var bestOffset = -1;
    ...
    for (var i = 0; i < entryCfis.length; i++) {
      ...
      // Try CFI resolution
      var entryRange = resolveRange(entryCfis[i]);
      if (entryRange) {
        entryOffset = globalCharOffset(entryRange);
      }

      // Fallback: find entry text in fullText
      if (entryOffset < 0 && entryTexts && i < entryTexts.length) {
        entryOffset = fullText.indexOf(entryTexts[i]);
      }

      var method = entryRange ? 'CFI' : 'text';
      console.log('[EPUB]   entry[' + i + '] offset=' + entryOffset + ' (' + method + ')');

      if (entryOffset >= 0 && entryOffset <= selectedOffset && entryOffset > bestOffset) {
        bestIdx = i;
        bestOffset = entryOffset;
      }
    }

    console.log('[EPUB] findMatchingAudioEntry: bestIdx=' + bestIdx);
    return bestIdx;
  } catch(e) {
 ...
 * @param {boolean} hasPrevious - Whether previous article is available
 * @param {boolean} hasNext - Whether next article is available
 */
function addNavigationButtons(hasPrevious, hasNext) {
  ...
  rendition.getContents().forEach(function (contents) {
    ...
    var existingNav = doc.getElementById('epub-nav-buttons');
    if (existingNav) {
      existingNav.remove();
    }

    // Create navigation buttons container
    var navContainer = doc.createElement('div');
    navContainer.id = 'epub-nav-buttons';
    ...
    var prevButton = doc.createElement('button');
    ...
    prevButton.style.cssText = `
      ...
      background-color: ${hasPrevious ? '#FF9945' : '#FBC9A0'};
      color: white;
      font-weight: 500;
    ...
    if (hasPrevious) {
      prevButton.addEventListener('click', function (e) {
        e.preventDefault();
        e.stopPropagation();
        console.log('[EPUB] Previous button clicked');
        window.flutter_inappwebview.callHandler('onPreviousArticle');
      });
    ...
    nextButton.style.cssText = `
      ...
      background-color: ${hasNext ? '#FF9945' : '#FBC9A0'};
      color: white;
      font-weight: 500;
      transition: opacity 0.2s;
    ...
    if (hasNext) {
      nextButton.addEventListener('click', function (e) {
        e.preventDefault();
        e.stopPropagation();
        console.log('[EPUB] Next button clicked');
        window.flutter_inappwebview.callHandler('onNextArticle');
      });
...
function removeNavigationButtons() {
  console.log('[EPUB] removeNavigationButtons');
