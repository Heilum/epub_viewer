...
function loadBook(data, cfi, manager, flow, spread, snap, allowScriptedContent, direction, useCustomSwipe, backgroundColor, foregroundColor, fontSize, clearSelectionOnNav) {
  ...
  rendition.hooks.content.register(function (contents) {
    contents.window.document.addEventListener('selectionchange', function () {
      ...
      console.log('[EPUB] 选择改变: ', selectedText, 'lastCfiRange: ', lastCfiRange);
      ...
      } else if (lastCfiRange) {
        // Selection is being modified (dragging handles)
        // Send intermediate selection updates via 'selectionChanging'
        //sendSelectionData(lastCfiRange, contents, 'selectionChanging');
        window.flutter_inappwebview.callHandler('selectionChanging');
      } else {
        selectionHasEnded = false;
        window.flutter_inappwebview.callHandler('seletionStart');
      }
    });

    // Explicitly detect selection end via mouseup/touchend so we can fire
    // a single "final" selection callback when the user lifts their finger.
    function handleSelectionEnd() {
      console.log('[EPUB] handleSelectionEnd: ', lastCfiRange);
      if (!lastCfiRange) {
        return;
      }
      var selection = contents.window.getSelection();
      console.log('[EPUB] selection: ', selection);
      if (!selection || !selection.toString()) {
        return;
      }

      // Fire one final selection event with the last CFI range
      sendSelectionData(lastCfiRange, contents, 'selectionEnd');
      selectionHasEnded = true;
    }

    function handleSelectionStart() {
      console.log('[EPUB] handleSelectionStart: ', lastCfiRange);
    ...
    contents.document.addEventListener('mouseup', handleSelectionEnd, true);
    contents.document.addEventListener('touchend', handleSelectionEnd, true);

    contents.document.addEventListener('mousedown', handleSelectionStart, true);
    contents.document.addEventListener('touchstart', handleSelectionStart, true);
  ...
  rendition.on("selected", function (cfiRange, contents) {
    ...
    console.log('[EPUB] JS的 selected: ', cfiRange, 'contents: ', contents);
  ...
  rendition.on("relocated", function (location) {
    ...
    if (clearSelectionOnPageChange && (lastCfiRange)) {
      lastCfiRange = null;
      if (selectionTimeout) {
        clearTimeout(selectionTimeout);
        selectionTimeout = null;
