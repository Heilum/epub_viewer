...
function loadBook(data, cfi, manager, flow, spread, snap, allowScriptedContent, direction, useCustomSwipe, backgroundColor, foregroundColor, fontSize, clearSelectionOnNav, axis, fontFamily, margin, pendingInitialSettings) {
  ...
  book.ready.then(async function () {
    ...
    if (initialCfi || initialPercentage) {
      ...
      setTimeout(function () {
        // Strategy: Use percentage for layout-independent positioning
        let canSet = initialPercentage !== null && initialPercentage !== undefined && book.locations && book.locations.length() > 0;
        console.log('[EPUB] canSet check - initialPercentage:', initialPercentage, 'book.locations:', !!book.locations, 'length:', book.locations ? book.locations.length() : 0);

        if (canSet) {
          console.log('[EPUB] Restoring position using percentage:', initialPercentage);

          try {
            var targetLocation = book.locations.cfiFromPercentage(initialPercentage);
            console.log('[EPUB] Calculated target CFI from percentage:', targetLocation);

            // Collapse range CFI to single point for more accurate positioning
            // Range CFI format: epubcfi(/6/2!/4,/20/1:301,/28/1:255)
            // Single point CFI: epubcfi(/6/2!/4/20/1:301)
            var displayCfi = targetLocation;
            if (targetLocation && targetLocation.indexOf(',') > -1) {
              try {
                // Manually parse range CFI to extract start point
                // Format: epubcfi(base,start,end) -> epubcfi(base+start)
                var match = targetLocation.match(/^(epubcfi\([^,]+),([^,]+),([^)]+)\)$/);
                if (match) {
                  var base = match[1];  // epubcfi(/6/2!/4
                  var start = match[2]; // /20/1:301
                  displayCfi = base + start + ')';
                  console.log('[EPUB] Collapsed CFI to single point:', displayCfi);
                } else {
                  console.warn('[EPUB] Could not parse range CFI, using original');
                }
              } catch (collapseErr) {
                console.warn('[EPUB] Failed to collapse CFI, using original:', collapseErr);
              }
            }

            rendition.display(displayCfi).then(function () {
              console.log('[EPUB] Position restored via percentage');
            }).catch(function (err) {
              console.error('[EPUB] Failed to restore via percentage, falling back to CFI:', err);
              if (initialCfi) {
                rendition.display(initialCfi);
              }
            });
          } catch (err) {
            console.error('[EPUB] Error calculating CFI from percentage:', err);
            if (initialCfi) {
              console.log('[EPUB] Falling back to CFI restoration');
              rendition.display(initialCfi);
            }
          }
        } else if (initialCfi) {
          var reason = !book.locations ? 'locations not available' :
            book.locations.length() === 0 ? 'locations not generated' :
              'percentage not provided';
          console.log('[EPUB] Restoring position using CFI (' + reason + '):', initialCfi);
          rendition.display(initialCfi);
          console.log('[EPUB] CFI restored');
        } else {
          console.log('[EPUB] No position to restore');
        }
...
function setFontSize(fontSize) {
  ...
  if (rendition && rendition.location && rendition.location.start) {
    ...
    console.log('[EPUB] Saving position before font size change:', currentPercentage);
  }
  ...
  // Regenerate locations and restore position after layout stabilizes
  setTimeout(function () {
    if (currentPercentage !== null && currentPercentage !== undefined && initialSettingsApplied) {
      console.log('[EPUB] Regenerating locations after font size change...');
      book.locations.generate(LOCATION_GENERATION_GRANULARITY).then(function () {
        console.log('[EPUB] Locations regenerated:', book.locations.length(), 'locations');

        // Restore to the same percentage position
        try {
          var targetCfi = book.locations.cfiFromPercentage(currentPercentage);
          console.log('[EPUB] Restoring position to', currentPercentage, 'via CFI:', targetCfi);

          // Collapse range CFI to single point
          var displayCfi = targetCfi;
          if (targetCfi && targetCfi.indexOf(',') > -1) {
            var match = targetCfi.match(/^(epubcfi\([^,]+),([^,]+),([^)]+)\)$/);
            if (match) {
              displayCfi = match[1] + match[2] + ')';
            }
          }

          rendition.display(displayCfi).then(function () {
            console.log('[EPUB] Position restored after font size change');
          });
        } catch (err) {
          console.error('[EPUB] Failed to restore position after font size change:', err);
        }
      }).catch(function (err) {
        console.error('[EPUB] Failed to regenerate locations:', err);
      });
    }
...
function setLineSpacing(spacing) {
  // Save current position before changing line spacing
  var currentPercentage = null;
  if (rendition && rendition.location && rendition.location.start) {
    currentPercentage = rendition.location.start.percentage;
    console.log('[EPUB] Saving position before line spacing change:', currentPercentage);
  }
  ...
  setTimeout(function () {
    if (currentPercentage !== null && currentPercentage !== undefined && initialSettingsApplied) {
      console.log('[EPUB] Regenerating locations after line spacing change...');
      book.locations.generate(LOCATION_GENERATION_GRANULARITY).then(function () {
        console.log('[EPUB] Locations regenerated:', book.locations.length(), 'locations');
        ...
        try {
          ...
          console.error('[EPUB] Failed to restore position after line spacing change:', err);
        }
      }).catch(function (err) {
        console.error('[EPUB] Failed to regenerate locations:', err);
      });
...
function setFontFamily(fontFamily) {
  ...
  return loadFontAsDataURI(fontFamily).then(dataURI => {
    if (rendition && rendition.themes) {
      ...
      setTimeout(function () {
        if (currentPercentage !== null && currentPercentage !== undefined && initialSettingsApplied) {
          console.log('[EPUB] Regenerating locations after font family change...');
          book.locations.generate(LOCATION_GENERATION_GRANULARITY).then(function () {
            console.log('[EPUB] Locations regenerated:', book.locations.length(), 'locations');
            ...
            try {
              ...
              console.error('[EPUB] Failed to restore position after font family change:', err);
            }
          }).catch(function (err) {
            console.error('[EPUB] Failed to regenerate locations:', err);
          });
